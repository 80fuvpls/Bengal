...
function createTextBand({
  ...
  // repeat đúng cách
  const repeatCount =
    Math.ceil(canvas.width / textWidth) + 1;
...
function animate() {
  ...
  currentZ +=
    (targetZ - currentZ) * 0.06;

  camera.position.z = currentZ;

  currentFov +=
    (targetFov - currentFov) * 0.06;
  ...
  group.children.forEach(mesh => {
    ...
    if (mesh.userData.fadeIn) {
      ...
      if (
        ...
        mesh.scale.x +=
          (targetScale - mesh.scale.x) * 0.06;

        mesh.scale.y +=
          (targetScale - mesh.scale.y) * 0.06;
